...
#Creating an ID clause on inpatient provider number to join with the main provider file
df_inpatient_providerlist <- sqldf("Select distinct PRVDR_NUM from df_inpatient")
df_inpatient_providerlist$ProviderID <- seq_len(nrow(df_inpatient_providerlist))
...
FROM
df_inpatient a
  left join df_inpatient_providerlist b on a.PRVDR_NUM = b.PRVDR_NUM
order by DESYNPUF_ID, CLM_ADMSN_DT
")

#Aggregated Provider Listing
df_inpatient_provideragg <- sqldf("Select ProviderID, count(distinct DESYNPUF_ID) as TotalInpatientCount,
                                                      count(distinct case when ReadmissionDays <=30 then DESYNPUF_ID end) TotalReadmissionInpatients
                                                                  from df_inpatient group by ProviderID")
...
## Using the 2010 PSBS PUF File
#Read all the Provider Files
df_provider0 <- fread("C:/Work/CMS_RArchitecture/Data/OtherData/2010 PSBS PUF.csv")
df_providerspecialitycodes <- fread("C:/Work/CMS_RArchitecture/Data/OtherData/ProviderSpecialityCodes.csv")

df_provider0$ProviderID <- seq_len(nrow(df_provider0))


df_provider <- sqldf("Select a.providerID, a.provider_state_of_service, a.nppes_entity_type_cd, a.nppes_gender,
a.nppes_soleprop_ind,   a.provider_specialty, c.Description as ProviderSpeciality, substr(a.nppes_provider_practice_zip, 5) as provider_zip, 
                      sum(a.bene_cnt) as bene_count, sum(a.death_bene_cnt) bene_death_count, sum(a.total_events) totalevents, 
                      sum(b.TotalInpatientCount) TotalInpatientCount, sum(b.TotalReadmissionInpatients) TotalReadmissionInpatients
                      from df_provider0 a
                            left join df_inpatient_provideragg b on a.ProviderID = b.ProviderID 
                            left join df_providerspecialitycodes c on a.provider_specialty = c.Code
                     group by a.ProviderID, a.provider_state_of_service, a.nppes_entity_type_cd, a.nppes_gender, a.nppes_soleprop_ind, a.provider_specialty,
                     a.nppes_provider_practice_zip, c.Description
                     ")

fwrite(df_provider, 'C:/Work/CMS_RArchitecture/Data/df_provider.csv')
